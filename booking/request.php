...
if ($_POST) {
    ...
    if ((!$checkcode) || (mysqli_num_rows($checkcode) > 0)) {
        $message .= "The Order Reference #$order Received belongs to another customer, Click <a href='index.php.php'>here</a> to generate another one. ";
    }
    ...
    if (empty($message)) {
        ...
        mysqli_query($conn, "UPDATE booking_details SET status='Pending' WHERE order_ref='$order'");
        if ($insertdata) {
            $message = "success";
        } else {
